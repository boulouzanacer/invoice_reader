        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="mt-4 sm:mt-0 w-full sm:w-auto">
                    <form id="clientSearchForm" method="GET" action="{{ route('clients.index') }}" class="flex flex-col sm:flex-row sm:items-center gap-3">
                        <div class="relative w-full sm:w-80">
                            ...
                            <input id="clientSearchInput" type="text" name="search" value="{{ $search ?? request('search') }}" placeholder="Search by name, username, phone" class="block w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @if(($search ?? request('search')) !== null && trim((string)($search ?? request('search'))) !== '')
                                <a href="{{ route('clients.index') }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600" aria-label="Clear search">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-100">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            <div class="text-xs text-gray-500 font-mono">{{ $client->username }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="space-y-2">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <code class="text-xs font-mono bg-gray-100 px-2 py-1 rounded text-gray-700 truncate max-w-[140px]">{{ $client->username }}</code>
                                            <div x-data="{ copied: false }">
                                                <button
                                                    @click="navigator.clipboard.writeText('{{ $client->username }}'); copied = true; setTimeout(() => copied = false, 1200)"
                                        ...
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <code class="text-xs font-mono bg-gray-100 px-2 py-1 rounded text-gray-700 truncate max-w-[140px]">{{ $client->password }}</code>
                                            <div x-data="{ copied: false }">
                                                <button
                                                    @click="navigator.clipboard.writeText('{{ $client->password }}'); copied = true; setTimeout(() => copied = false, 1200)"
                                                    class="text-gray-400 hover:text-blue-600 transition-colors focus:outline-none"
                                                    title="Copy Password"
                                                >
                                                    <template x-if="!copied">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <div class="flex items-center justify-between text-xs mb-1">
                                            <span class="font-medium {{ $client->remaining_keywords < 10 ? 'text-red-600' : 'text-gray-900' }}">{{ $client->remaining_keywords }}</span>
                                            <span class="text-gray-400">/ {{ $client->keyword_count }}</span>
                                        </div>
                                        <div class="w-24 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                            ...
                                            @endphp
                                            <div class="h-full {{ $color }} rounded-full" style="width: {{ min(100, $percentage) }}%"></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form action="{{ route('clients.toggle', $client) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 {{ $client->is_enabled ? 'bg-green-500' : 'bg-gray-200' }}">
                                            <span class="sr-only">Use setting</span>
                                            <span aria-hidden="true" class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 {{ $client->is_enabled ? 'translate-x-5' : 'translate-x-0' }}"></span>
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-3">
                                        <button onclick="editClient({{ $client->id }})" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 p-1.5 rounded-md hover:bg-indigo-100 transition-colors">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="md:hidden bg-gray-50 p-4 space-y-4">
                    @foreach($clients as $client)
                    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 transition-shadow hover:shadow-md {{ !$client->is_enabled ? 'opacity-75' : '' }}">
                        <div class="flex justify-between items-start mb-4">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <h4 class="font-bold text-gray-900 text-lg leading-tight">{{ $client->name ?? 'Unnamed Client' }}</h4>
                                    <p class="text-xs text-gray-500 font-mono mt-0.5">{{ $client->username }}</p>
                                </div>
                            </div>
                            <form action="{{ route('clients.toggle', $client) }}" method="POST">
                                @csrf
                        ...
                        <div class="space-y-3 text-sm text-gray-600 mb-5">
                            ...
                            <div class="py-2 border-b border-gray-100">
                                <div class="flex justify-between mb-1">
                                    ...
                                    <span class="font-medium text-gray-900">{{ $client->remaining_keywords }} / {{ $client->keyword_count }}</span>
                                </div>
                            ...
                            <div class="py-2">
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-center space-x-2 bg-gray-50 p-2 rounded-lg border border-gray-100">
                                        ...
                                        <code class="text-xs flex-1 truncate font-mono text-gray-700 select-all">{{ $client->username }}</code>
                                        <div x-data="{ copied: false }">
                                            <button
                                                @click="navigator.clipboard.writeText('{{ $client->username }}'); copied = true; setTimeout(() => copied = false, 1200)"
                                    ...
                                    <div class="flex items-center space-x-2 bg-gray-50 p-2 rounded-lg border border-gray-100">
                                        ...
                                        <code class="text-xs flex-1 truncate font-mono text-gray-700 select-all">{{ $client->password }}</code>
                                        <div x-data="{ copied: false }">
                                            <button
                                                @click="navigator.clipboard.writeText('{{ $client->password }}'); copied = true; setTimeout(() => copied = false, 1200)"
                                                class="p-1.5 bg-white border border-gray-200 rounded-md text-gray-500 hover:text-blue-600 shadow-sm focus:outline-none"
                        ...
                        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                            <button onclick="editClient({{ $client->id }})" class="flex-1 inline-flex justify-center items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none">
                                <svg class="h-4 w-4 mr-2 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                                Edit
                            </button>
                            <form action="{{ route('clients.destroy', $client) }}" method="POST" class="flex-1 inline-block">
                                @csrf
                                @method('DELETE')
    ...
    <script>
        ...
        function openModal(data = null) {
            ...
            if (data) {
                ...
                form.action = `{{ url('clients') }}/${data.id}`;
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                @if(old('_method') == 'PUT')
                    openModal({
                        ...
                        password: {!! json_encode(old('password')) !!},
                        name: {!! json_encode(old('name')) !!},
                        address: {!! json_encode(old('address')) !!},
                        phone: {!! json_encode(old('phone')) !!},
                        keyword_count: {!! json_encode(old('keyword_count')) !!},
                        remaining_keywords: {!! json_encode(old('remaining_keywords') ?? 0) !!},
                        is_enabled: {{ old('is_enabled') ? 'true' : 'false' }}
                    });
                @else
                    // Open modal for CREATE action and pre-fill old values
                    openModal();
                    // Manually fill fields since openModal() resets them for new entries
                    document.getElementById('client_username').value = {!! json_encode(old('username')) !!};
                    document.getElementById('client_password').value = {!! json_encode(old('password')) !!};
                    document.getElementById('client_name').value = {!! json_encode(old('name')) !!};
                    document.getElementById('client_address').value = {!! json_encode(old('address')) !!};
                    document.getElementById('client_phone').value = {!! json_encode(old('phone')) !!};
                    document.getElementById('client_keyword_count').value = {!! json_encode(old('keyword_count')) !!};
                    document.getElementById('client_is_enabled').checked = {{ old('is_enabled') ? 'true' : 'true' }}; // Default true for new
                @endif
            });
        @endif
