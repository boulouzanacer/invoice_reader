        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="mt-4 sm:mt-0 w-full sm:w-auto">
                    <form id="quotaSearchForm" method="GET" action="{{ route('quotas.index') }}" class="flex flex-col sm:flex-row sm:items-center gap-3">
                        <div class="relative w-full sm:w-72">
                            ...
                            <input id="quotaSearchInput" type="text" name="search" value="{{ $search ?? request('search') }}" placeholder="Rechercher par client, username, téléphone" class="block w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @if(($search ?? request('search')) !== null && trim((string)($search ?? request('search'))) !== '')
                                <a href="{{ route('quotas.index', array_filter(['date' => $date ?? request('date')])) }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600" aria-label="Clear search">
                        ...
                        <div class="relative w-full sm:w-52">
                            <input id="quotaDateInput" type="date" name="date" value="{{ $date ?? request('date') }}" class="block w-full py-2.5 px-3 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        </div>
                            ...
                            @foreach($quotas as $quota)
                                <tr class="hover:bg-gray-50 transition-colors duration-150 group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold text-lg">
                                                {{ substr($quota->client->name ?? 'C', 0, 1) }}
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $quota->client->name ?? 'Unnamed Client' }}</div>
                                                <div class="text-xs text-gray-500">{{ $quota->client->phone ?? '-' }}</div>
                                                <div class="text-xs text-gray-500 font-mono">{{ $quota->client->username ?? '-' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $quota->quota_date?->format('Y-m-d') ?? $quota->quota_date }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-3">
                                            <button type="button"
                                                    onclick='openQuotaModal({{ json_encode(["id"=>$quota->id,"client_id"=>$quota->client_id,"client_name"=>$quota->client->name,"client_username"=>$quota->client->username,"amount"=>$quota->amount,"quota_date"=>(string)$quota->quota_date]) }})'
                                            ...
                                            <form action="{{ route('quotas.destroy', $quota) }}" method="POST" class="inline" onsubmit="return confirm('Confirmer la suppression de ce quota ?');">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 transition-shadow hover:shadow-md">
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-gray-900 text-lg leading-tight">{{ $quota->client->name ?? 'Unnamed Client' }}</h4>
                                        <p class="text-xs text-gray-500 mt-0.5">{{ $quota->client->phone ?? '-' }}</p>
                                        <p class="text-xs text-gray-500 font-mono mt-0.5">{{ $quota->client->username ?? '-' }}</p>
                                    </div>
                                </div>
                                <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 text-sm font-semibold">
                                    +{{ $quota->amount }}
                            ...
                            <div class="text-sm text-gray-600 mb-4">
                                <div class="flex justify-between py-2 border-b border-gray-100">
                                    ...
                                    <span class="font-medium">{{ $quota->quota_date?->format('Y-m-d') ?? $quota->quota_date }}</span>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                                ...
                                <form action="{{ route('quotas.destroy', $quota) }}" method="POST" class="flex-1 inline-block" onsubmit="return confirm('Confirmer la suppression de ce quota ?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <div id="quotaModal" class="fixed z-20 inset-0 overflow-y-auto hidden" aria-labelledby="quota-modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full border border-gray-100">
                <form id="quotaForm" action="{{ route('quotas.store') }}" method="POST">
                    @csrf
                            ...
                            <div>
                                ...
                                <select name="client_id" id="quota_client_id" required class="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-lg">
                                    ...
                                    @foreach($clients as $client)
                                        <option value="{{ $client->id }}">{{ $client->name ?? 'Unnamed' }} ({{ $client->username }})</option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <script>
        ...
        function openQuotaModal(data = null) {
            ...
            if (data) {
                ...
                form.action = `{{ url('quotas') }}/${data.id}`;
                ...
                form.action = "{{ route('quotas.store') }}";
        ...
        @if ($errors->any())
            document.addEventListener('DOMContentLoaded', function() {
                ...
                @if(old('_method') == 'PUT' && old('quota_id'))
                    openQuotaModal({
                        id: {!! json_encode(old('quota_id')) !!},
                        client_id: {!! json_encode(old('client_id')) !!},
                        amount: {!! json_encode(old('amount')) !!},
                        quota_date: {!! json_encode(old('quota_date') ?? now()->toDateString()) !!}
                    });
                @else
                    openQuotaModal();
                    document.getElementById('quota_client_id').value = {!! json_encode(old('client_id')) !!};
                    document.getElementById('quota_amount').value = {!! json_encode(old('amount')) !!};
                    const dateValue = {!! json_encode(old('quota_date') ?? now()->toDateString()) !!};
